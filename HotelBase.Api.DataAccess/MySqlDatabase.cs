...
namespace LeaRun.Data.Dapper
{
    ...
    public class MySqlDatabase
    {
        ...
        public int Update<T>(T entity,bool isBatch = false) where T : class
        {
        ...
        public int UpdateNew<T>(IEnumerable<T> entities) where T : class
        {
            if (dbTransaction == null)
            {
                BeginTrans();
        ...
        public DataTable GetTable(string strSql, DbParameter[] dbParameter)
        {
            using (var dbConnection = Connection)
            {
                var IDataReader = new DbHelper(dbConnection).ExecuteReader(CommandType.Text, strSql, dbParameter);
                return ConvertExtension.IDataReaderToTable(IDataReader);
            }
        }
        public DataTable GetTable(string strSql, string orderField, bool isAsc, int pageSize, int pageIndex, out int total)
        {
            return GetTable(strSql, null, orderField, isAsc, pageSize, pageIndex, out total);
        }
        public DataTable GetTable(string strSql, DbParameter[] dbParameter, string orderField, bool isAsc, int pageSize, int pageIndex, out int total)
        {
            using (var dbConnection = Connection)
            {
                StringBuilder sb = new StringBuilder();
                ...
                total = Convert.ToInt32(new DbHelper(dbConnection).ExecuteScalar(CommandType.Text, "Select Count(1) From (" + strSql + ") As t", dbParameter));
                var IDataReader = new DbHelper(dbConnection).ExecuteReader(CommandType.Text, sb.ToString(), dbParameter);
                DataTable resultTable = ConvertExtension.IDataReaderToTable(IDataReader);
                return resultTable;
            }
        }
